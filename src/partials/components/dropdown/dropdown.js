...
function isEmptyValues(dropdown) {
  const sum = Object.values(getData(dropdown)).reduce(
...
$dropInputs.each(function () {
  switch (this.value) {
    case this.min:
      this.previousElementSibling.classList.add("nice-number__button_disabed");
      break;
    case this.max:
      this.nextElementSibling.classList.add("nice-number__button_disabed");
      break;
  }
});
